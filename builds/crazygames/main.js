...
const i18n = {
  pt: {
    ...
    donateBtn: "🥣 Alimente Nossos Bichinhos",
    donateTitle: "AJUDE A ENCHER AS TIGELAS! 🐾",
  ...
  en: {
    ...
    donateBtn: "🥣 Feed Our Real Pets",
    donateTitle: "HELP US FILL THEIR BOWLS! 🐾",
  ...
  es: {
    ...
    donateBtn: "🥣 Alimenta a Nuestras Mascotas",
    donateTitle: "¡AYUDANOS A LLENAR LOS PLATOS! 🐾",
...
let currentLang = (window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_lang') : localStorage.getItem('fridaArcade_lang')) || navigator.language.substring(0, 2) || 'en';
...
function translateUI() {
  ...
  if (window.MonetizationManager) window.MonetizationManager.setItem('fridaArcade_lang', currentLang);
  else localStorage.setItem('fridaArcade_lang', currentLang);
...
// --- GLOBAL GAME ECONOMY ---
let globalCoins = parseInt((window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_coins') : localStorage.getItem('fridaArcade_coins')) || '0');
let unlockedHeroes = JSON.parse((window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_unlocked') : localStorage.getItem('fridaArcade_unlocked')) || '["luna", "frida"]');
...
function updateCoinsDisplay() {
  ...
  if (window.MonetizationManager) window.MonetizationManager.setItem('fridaArcade_coins', globalCoins);
  else localStorage.setItem('fridaArcade_coins', globalCoins);
}
...
function updateHighScores(newScore, playerName) {
    const savedStr = (window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_high_scores') : localStorage.getItem('fridaArcade_high_scores'));
    let scores = JSON.parse(savedStr || '[]');
    ...
    const finalStr = JSON.stringify(scores);
    if (window.MonetizationManager) window.MonetizationManager.setItem('fridaArcade_high_scores', finalStr);
    else localStorage.setItem('fridaArcade_high_scores', finalStr);
}

function renderHighScores() {
    const savedScores = (window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_high_scores') : localStorage.getItem('fridaArcade_high_scores'));
...
function saveGame() {
  ...
  if (window.MonetizationManager) window.MonetizationManager.setItem('fridaArcade_save', dataStr);
  else localStorage.setItem('fridaArcade_save', dataStr);
}

// Essa função lê o caderninho quando você abre o jogo
function loadGame() {
  const saved = (window.MonetizationManager ? window.MonetizationManager.getItem('fridaArcade_save') : localStorage.getItem('fridaArcade_save'));
  if (saved) {
    try {
    ...
    if (!unlockedHeroes.includes(val)) {
      ...
      if (confirm(msg)) {
        if (globalCoins >= cost) {
          ...
          if (window.MonetizationManager) window.MonetizationManager.setItem('fridaArcade_unlocked', JSON.stringify(unlockedHeroes));
          else localStorage.setItem('fridaArcade_unlocked', JSON.stringify(unlockedHeroes));
        } else {
...
if(openDonationBtn) {
  openDonationBtn.addEventListener('click', (e) => {
    e.preventDefault();
...
if(closeDonationBtn) {
  closeDonationBtn.addEventListener('click', (e) => {
    e.preventDefault();
...
if(copyPixBtn) {
  copyPixBtn.addEventListener('click', (e) => {
    e.preventDefault();
    const pixKey = "657101db-a06a-409f-a840-e40649a0b943";
    navigator.clipboard.writeText(pixKey).then(() => {
      const originalText = copyPixBtn.innerText;
      copyPixBtn.innerText = "¡Copiado! / Copied! / Copiado!";
      setTimeout(() => {
